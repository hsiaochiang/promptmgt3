...
- Q: 搜尋（FR-005）的預設範圍涵蓋哪些資料？ → A: 僅搜尋主資產 Active：Project/Prompt 且 `archived=false`；不含 Trash、不含 Inbox。
- Q: 拖曳圖片/附件上傳時，附件檔名策略？ → A: 產生 UUID 檔名（保留副檔名）。
- Q: Project/Prompt 的身分識別與檔案路徑策略？ → A: `id=UUID`（存於 frontmatter）；路徑使用 `slug`（可改名）；rename/restore 只改 slug，不改 id。
- Q: Project 與 Prompt 的落盤佈局？ → A: Prompt 檔案放在 Project 資料夾底下（階層式），例如 `projects/<projectSlug>/prompts/<promptSlug>.md`。
- Q: `slug` 的生成/正規化規則？ → A: 僅允許 ASCII kebab-case（`a-z0-9-`），空白轉 `-`、連續 `-` 合併，其他字元轉換或移除。
...
3. **前提** 使用者想移除不需要的暫存項目，**當** 對該暫存項目執行刪除並確認，**則** 該項目自暫存區移除且顯示成功回饋；此刪除為永久刪除（不可於回收站復原）；若刪除失敗，需顯示可理解錯誤並允許重試。
...
- **FR-004**: 支援拖曳圖片/附件到編輯區，系統需移動至附件目錄、產生檔名並插入引用連結。
	- 檔名策略：上傳附件一律產生 UUID 檔名並保留副檔名。
- **FR-005**: 提供搜尋與篩選：可依標題、標籤、正文全文檢索，並以狀態、優先級、標籤快速篩選；結果應可直接開啟詳情。
	- 預設搜尋範圍：僅 Project/Prompt 且 `archived=false`；不含 Trash、不含 Inbox。
- **FR-006**: 暫存區必須顯示匯入項目並允許編輯標題/刪減/註記，清楚提示「歸檔由外部工具處理」，不提供歸檔操作。
	- 允許刪除暫存項目（InboxItem delete）：代表「不再保留此暫存項」，需提供明確確認與可理解回饋。
		- InboxItem delete 為永久刪除：不進回收站（`<rootPath>/.pah/trash/`）且不可復原。
...
- **Project**: 識別碼、標題、摘要、狀態、專案類型、標籤、關聯檔案、更新時間、封存狀態。
	- 識別碼（id）：UUID（存於 frontmatter），不可因改名而改變。
	- 路徑識別（slug）：用於資料夾/檔名，可改名；改名只影響 slug，不影響 id。
		- slug 規則：僅允許 ASCII kebab-case（`a-z0-9-`）；空白→`-`、連續 `-` 合併；其他字元轉換或移除。
	- 落盤佈局：Prompt 檔案位於 Project 資料夾內（階層式）；例如 `projects/<projectSlug>/prompts/<promptSlug>.md`。
	- status: `planned | in_progress | paused | done`
- **Prompt**: 識別碼、所屬專案、標題、狀態（草稿/待優化/可用/停用）、優先級、標籤、來源連結、備註、正文、更新時間、封存狀態。
	- 識別碼（id）：UUID（存於 frontmatter），不可因改名而改變。
	- 路徑識別（slug）：用於檔名，可改名；改名只影響 slug，不影響 id。
		- slug 規則：僅允許 ASCII kebab-case（`a-z0-9-`）；空白→`-`、連續 `-` 合併；其他字元轉換或移除。
	- 落盤佈局：位於所屬 Project 資料夾下的 `prompts/` 子資料夾。
